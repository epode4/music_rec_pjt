...
def find_top_similar_in_cluster(current_user_top2):
    ...
    for current_user_top in current_user_top2:
        ...
        sample_row_queryset = AudioFeature.objects.filter(track_id=current_user_top)
        sample_row_values = sample_row_queryset.values('track_id','pca_x', 'pca_y', 'gmm_cluster', 'danceability', 'energy', 'loudness', 'acousticness', 'tempo')
        ...
        # 같은 군집 내의 음원들만 선택        
        df_same_cluster = AudioFeature.objects.filter(gmm_cluster=sample_cluster).values('track_id', 'pca_x', 'pca_y', 'gmm_cluster', 'danceability', 'energy', 'loudness', 'acousticness', 'tempo')
        df_same_cluster = pd.DataFrame.from_records(df_same_cluster)


        # 선택한 음원과 같은 군집 내 음원들의 벡터 데이터
        df_sample_vector = sample_row[['danceability', 'energy', 'loudness', 'acousticness', 'tempo']].values
        df_same_cluster_vector = df_same_cluster[['danceability', 'energy', 'loudness', 'acousticness', 'tempo']].values
        print('vector : 출력됨')
        ...
        # 가장 유사한 곡 2개 선택
        similar1 = np.argsort(cosine_similarities, axis=1)[:, ::-1][:, 1:16] 
